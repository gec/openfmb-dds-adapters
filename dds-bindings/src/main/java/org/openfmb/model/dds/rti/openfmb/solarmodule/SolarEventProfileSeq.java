...
/**
* A sequence of SolarEventProfile instances.
*/
public final class SolarEventProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SolarEventProfileSeq() {
        super(SolarEventProfile.class);
    }

    public SolarEventProfileSeq (int initialMaximum) {
        super(SolarEventProfile.class, initialMaximum);
    }

    public SolarEventProfileSeq (Collection elements) {
        super(SolarEventProfile.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SolarEventProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SolarEventProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SolarEventProfile.create());
