...
/**
* A sequence of ResourceReadingProfile instances.
*/
public final class ResourceReadingProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ResourceReadingProfileSeq() {
        super(ResourceReadingProfile.class);
    }

    public ResourceReadingProfileSeq (int initialMaximum) {
        super(ResourceReadingProfile.class, initialMaximum);
    }

    public ResourceReadingProfileSeq (Collection elements) {
        super(ResourceReadingProfile.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ResourceReadingProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ResourceReadingProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ResourceReadingProfile.create());
