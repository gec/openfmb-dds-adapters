...
/**
* A sequence of SequenceOfOptimizedMicroGridMarketMarketFactors instances.
*/
public final class SequenceOfOptimizedMicroGridMarketMarketFactorsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SequenceOfOptimizedMicroGridMarketMarketFactorsSeq() {
        super(SequenceOfOptimizedMicroGridMarketMarketFactors.class);
    }

    public SequenceOfOptimizedMicroGridMarketMarketFactorsSeq (int initialMaximum) {
        super(SequenceOfOptimizedMicroGridMarketMarketFactors.class, initialMaximum);
    }

    public SequenceOfOptimizedMicroGridMarketMarketFactorsSeq (Collection elements) {
        super(SequenceOfOptimizedMicroGridMarketMarketFactors.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SequenceOfOptimizedMicroGridMarketMarketFactors.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SequenceOfOptimizedMicroGridMarketMarketFactors objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SequenceOfOptimizedMicroGridMarketMarketFactors.create());
