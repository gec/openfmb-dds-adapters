...
/**
* A sequence of BatteryEventProfile instances.
*/
public final class BatteryEventProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BatteryEventProfileSeq() {
        super(BatteryEventProfile.class);
    }

    public BatteryEventProfileSeq (int initialMaximum) {
        super(BatteryEventProfile.class, initialMaximum);
    }

    public BatteryEventProfileSeq (Collection elements) {
        super(BatteryEventProfile.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, BatteryEventProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' BatteryEventProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(BatteryEventProfile.create());
