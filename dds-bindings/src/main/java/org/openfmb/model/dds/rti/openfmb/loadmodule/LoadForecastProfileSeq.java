...
/**
* A sequence of LoadForecastProfile instances.
*/
public final class LoadForecastProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public LoadForecastProfileSeq() {
        super(LoadForecastProfile.class);
    }

    public LoadForecastProfileSeq (int initialMaximum) {
        super(LoadForecastProfile.class, initialMaximum);
    }

    public LoadForecastProfileSeq (Collection elements) {
        super(LoadForecastProfile.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, LoadForecastProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' LoadForecastProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(LoadForecastProfile.create());
