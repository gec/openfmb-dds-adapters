...
/**
* A sequence of GenerationEventModule instances.
*/
public final class GenerationEventModuleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public GenerationEventModuleSeq() {
        super(GenerationEventModule.class);
    }

    public GenerationEventModuleSeq (int initialMaximum) {
        super(GenerationEventModule.class, initialMaximum);
    }

    public GenerationEventModuleSeq (Collection elements) {
        super(GenerationEventModule.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, GenerationEventModule.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' GenerationEventModule objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(GenerationEventModule.create());
