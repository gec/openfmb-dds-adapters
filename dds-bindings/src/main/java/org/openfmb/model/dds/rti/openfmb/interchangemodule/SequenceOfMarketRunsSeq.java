...
/**
* A sequence of SequenceOfMarketRuns instances.
*/
public final class SequenceOfMarketRunsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SequenceOfMarketRunsSeq() {
        super(SequenceOfMarketRuns.class);
    }

    public SequenceOfMarketRunsSeq (int initialMaximum) {
        super(SequenceOfMarketRuns.class, initialMaximum);
    }

    public SequenceOfMarketRunsSeq (Collection elements) {
        super(SequenceOfMarketRuns.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SequenceOfMarketRuns.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SequenceOfMarketRuns objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SequenceOfMarketRuns.create());
