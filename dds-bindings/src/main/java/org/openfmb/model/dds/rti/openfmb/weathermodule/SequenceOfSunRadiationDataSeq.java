...
/**
* A sequence of SequenceOfSunRadiationData instances.
*/
public final class SequenceOfSunRadiationDataSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SequenceOfSunRadiationDataSeq() {
        super(SequenceOfSunRadiationData.class);
    }

    public SequenceOfSunRadiationDataSeq (int initialMaximum) {
        super(SequenceOfSunRadiationData.class, initialMaximum);
    }

    public SequenceOfSunRadiationDataSeq (Collection elements) {
        super(SequenceOfSunRadiationData.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SequenceOfSunRadiationData.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SequenceOfSunRadiationData objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SequenceOfSunRadiationData.create());
