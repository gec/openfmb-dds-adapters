...
/**
* A sequence of SequenceOfCurveData instances.
*/
public final class SequenceOfCurveDataSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SequenceOfCurveDataSeq() {
        super(SequenceOfCurveData.class);
    }

    public SequenceOfCurveDataSeq (int initialMaximum) {
        super(SequenceOfCurveData.class, initialMaximum);
    }

    public SequenceOfCurveDataSeq (Collection elements) {
        super(SequenceOfCurveData.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SequenceOfCurveData.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SequenceOfCurveData objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SequenceOfCurveData.create());
