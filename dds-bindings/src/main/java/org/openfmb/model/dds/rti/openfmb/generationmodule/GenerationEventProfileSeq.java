...
/**
* A sequence of GenerationEventProfile instances.
*/
public final class GenerationEventProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public GenerationEventProfileSeq() {
        super(GenerationEventProfile.class);
    }

    public GenerationEventProfileSeq (int initialMaximum) {
        super(GenerationEventProfile.class, initialMaximum);
    }

    public GenerationEventProfileSeq (Collection elements) {
        super(GenerationEventProfile.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, GenerationEventProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' GenerationEventProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(GenerationEventProfile.create());
