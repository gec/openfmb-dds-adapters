...
/**
* A sequence of Recloser instances.
*/
public final class RecloserSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RecloserSeq() {
        super(Recloser.class);
    }

    public RecloserSeq (int initialMaximum) {
        super(Recloser.class, initialMaximum);
    }

    public RecloserSeq (Collection elements) {
        super(Recloser.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, Recloser.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' Recloser objects (beyond the original size of this object)
