...
/**
* A sequence of LoadStatusProfile instances.
*/
public final class LoadStatusProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public LoadStatusProfileSeq() {
        super(LoadStatusProfile.class);
    }

    public LoadStatusProfileSeq (int initialMaximum) {
        super(LoadStatusProfile.class, initialMaximum);
    }

    public LoadStatusProfileSeq (Collection elements) {
        super(LoadStatusProfile.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, LoadStatusProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' LoadStatusProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(LoadStatusProfile.create());
