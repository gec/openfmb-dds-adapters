...
/**
* A sequence of GenerationReadingProfile instances.
*/
public final class GenerationReadingProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public GenerationReadingProfileSeq() {
        super(GenerationReadingProfile.class);
    }

    public GenerationReadingProfileSeq (int initialMaximum) {
        super(GenerationReadingProfile.class, initialMaximum);
    }

    public GenerationReadingProfileSeq (Collection elements) {
        super(GenerationReadingProfile.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, GenerationReadingProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' GenerationReadingProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(GenerationReadingProfile.create());
