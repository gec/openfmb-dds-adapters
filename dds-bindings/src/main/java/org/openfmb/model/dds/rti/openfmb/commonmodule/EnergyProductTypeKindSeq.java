...
/**
* A sequence of EnergyProductTypeKind instances.
*/
public final class EnergyProductTypeKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public EnergyProductTypeKindSeq() {
        super(EnergyProductTypeKind.class);
    }

    public EnergyProductTypeKindSeq (int initialMaximum) {
        super(EnergyProductTypeKind.class, initialMaximum);
    }

    public EnergyProductTypeKindSeq (Collection elements) {
        super(EnergyProductTypeKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, EnergyProductTypeKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' EnergyProductTypeKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(EnergyProductTypeKind.create());
