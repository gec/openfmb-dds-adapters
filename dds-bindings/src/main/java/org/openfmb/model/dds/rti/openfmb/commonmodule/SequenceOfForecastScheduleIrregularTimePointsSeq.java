...
/**
* A sequence of SequenceOfForecastScheduleIrregularTimePoints instances.
*/
public final class SequenceOfForecastScheduleIrregularTimePointsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SequenceOfForecastScheduleIrregularTimePointsSeq() {
        super(SequenceOfForecastScheduleIrregularTimePoints.class);
    }

    public SequenceOfForecastScheduleIrregularTimePointsSeq (int initialMaximum) {
        super(SequenceOfForecastScheduleIrregularTimePoints.class, initialMaximum);
    }

    public SequenceOfForecastScheduleIrregularTimePointsSeq (Collection elements) {
        super(SequenceOfForecastScheduleIrregularTimePoints.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SequenceOfForecastScheduleIrregularTimePoints.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SequenceOfForecastScheduleIrregularTimePoints objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SequenceOfForecastScheduleIrregularTimePoints.create());
