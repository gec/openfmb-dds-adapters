...
/**
* A sequence of MarketProductTypeKind instances.
*/
public final class MarketProductTypeKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public MarketProductTypeKindSeq() {
        super(MarketProductTypeKind.class);
    }

    public MarketProductTypeKindSeq (int initialMaximum) {
        super(MarketProductTypeKind.class, initialMaximum);
    }

    public MarketProductTypeKindSeq (Collection elements) {
        super(MarketProductTypeKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, MarketProductTypeKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' MarketProductTypeKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(MarketProductTypeKind.create());
