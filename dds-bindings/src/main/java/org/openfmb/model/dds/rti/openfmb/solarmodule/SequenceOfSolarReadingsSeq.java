...
/**
* A sequence of SequenceOfSolarReadings instances.
*/
public final class SequenceOfSolarReadingsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SequenceOfSolarReadingsSeq() {
        super(SequenceOfSolarReadings.class);
    }

    public SequenceOfSolarReadingsSeq (int initialMaximum) {
        super(SequenceOfSolarReadings.class, initialMaximum);
    }

    public SequenceOfSolarReadingsSeq (Collection elements) {
        super(SequenceOfSolarReadings.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SequenceOfSolarReadings.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SequenceOfSolarReadings objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SequenceOfSolarReadings.create());
