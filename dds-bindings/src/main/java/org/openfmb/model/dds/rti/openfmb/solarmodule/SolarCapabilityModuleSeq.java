...
/**
* A sequence of SolarCapabilityModule instances.
*/
public final class SolarCapabilityModuleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SolarCapabilityModuleSeq() {
        super(SolarCapabilityModule.class);
    }

    public SolarCapabilityModuleSeq (int initialMaximum) {
        super(SolarCapabilityModule.class, initialMaximum);
    }

    public SolarCapabilityModuleSeq (Collection elements) {
        super(SolarCapabilityModule.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SolarCapabilityModule.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SolarCapabilityModule objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SolarCapabilityModule.create());
