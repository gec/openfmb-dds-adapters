...
/**
* A sequence of BatteryControlModule instances.
*/
public final class BatteryControlModuleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BatteryControlModuleSeq() {
        super(BatteryControlModule.class);
    }

    public BatteryControlModuleSeq (int initialMaximum) {
        super(BatteryControlModule.class, initialMaximum);
    }

    public BatteryControlModuleSeq (Collection elements) {
        super(BatteryControlModule.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, BatteryControlModule.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' BatteryControlModule objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(BatteryControlModule.create());
