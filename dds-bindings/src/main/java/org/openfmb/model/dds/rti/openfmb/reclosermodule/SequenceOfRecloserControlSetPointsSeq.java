...
/**
* A sequence of SequenceOfRecloserControlSetPoints instances.
*/
public final class SequenceOfRecloserControlSetPointsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SequenceOfRecloserControlSetPointsSeq() {
        super(SequenceOfRecloserControlSetPoints.class);
    }

    public SequenceOfRecloserControlSetPointsSeq (int initialMaximum) {
        super(SequenceOfRecloserControlSetPoints.class, initialMaximum);
    }

    public SequenceOfRecloserControlSetPointsSeq (Collection elements) {
        super(SequenceOfRecloserControlSetPoints.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SequenceOfRecloserControlSetPoints.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SequenceOfRecloserControlSetPoints objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SequenceOfRecloserControlSetPoints.create());
