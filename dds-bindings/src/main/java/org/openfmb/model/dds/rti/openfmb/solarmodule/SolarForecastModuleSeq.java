...
/**
* A sequence of SolarForecastModule instances.
*/
public final class SolarForecastModuleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SolarForecastModuleSeq() {
        super(SolarForecastModule.class);
    }

    public SolarForecastModuleSeq (int initialMaximum) {
        super(SolarForecastModule.class, initialMaximum);
    }

    public SolarForecastModuleSeq (Collection elements) {
        super(SolarForecastModule.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SolarForecastModule.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SolarForecastModule objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SolarForecastModule.create());
