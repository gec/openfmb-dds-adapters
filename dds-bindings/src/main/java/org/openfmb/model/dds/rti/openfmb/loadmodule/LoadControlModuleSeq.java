...
/**
* A sequence of LoadControlModule instances.
*/
public final class LoadControlModuleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public LoadControlModuleSeq() {
        super(LoadControlModule.class);
    }

    public LoadControlModuleSeq (int initialMaximum) {
        super(LoadControlModule.class, initialMaximum);
    }

    public LoadControlModuleSeq (Collection elements) {
        super(LoadControlModule.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, LoadControlModule.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' LoadControlModule objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(LoadControlModule.create());
