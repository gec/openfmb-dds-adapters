...
/**
* A sequence of RecloserReadingModule instances.
*/
public final class RecloserReadingModuleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RecloserReadingModuleSeq() {
        super(RecloserReadingModule.class);
    }

    public RecloserReadingModuleSeq (int initialMaximum) {
        super(RecloserReadingModule.class, initialMaximum);
    }

    public RecloserReadingModuleSeq (Collection elements) {
        super(RecloserReadingModule.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RecloserReadingModule.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RecloserReadingModule objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RecloserReadingModule.create());
