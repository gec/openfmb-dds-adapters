...
/**
* A sequence of SequenceOfRecloserReadings instances.
*/
public final class SequenceOfRecloserReadingsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SequenceOfRecloserReadingsSeq() {
        super(SequenceOfRecloserReadings.class);
    }

    public SequenceOfRecloserReadingsSeq (int initialMaximum) {
        super(SequenceOfRecloserReadings.class, initialMaximum);
    }

    public SequenceOfRecloserReadingsSeq (Collection elements) {
        super(SequenceOfRecloserReadings.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SequenceOfRecloserReadings.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SequenceOfRecloserReadings objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SequenceOfRecloserReadings.create());
