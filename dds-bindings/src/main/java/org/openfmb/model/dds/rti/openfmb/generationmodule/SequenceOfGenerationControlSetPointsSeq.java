...
/**
* A sequence of SequenceOfGenerationControlSetPoints instances.
*/
public final class SequenceOfGenerationControlSetPointsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SequenceOfGenerationControlSetPointsSeq() {
        super(SequenceOfGenerationControlSetPoints.class);
    }

    public SequenceOfGenerationControlSetPointsSeq (int initialMaximum) {
        super(SequenceOfGenerationControlSetPoints.class, initialMaximum);
    }

    public SequenceOfGenerationControlSetPointsSeq (Collection elements) {
        super(SequenceOfGenerationControlSetPoints.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SequenceOfGenerationControlSetPoints.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SequenceOfGenerationControlSetPoints objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SequenceOfGenerationControlSetPoints.create());
