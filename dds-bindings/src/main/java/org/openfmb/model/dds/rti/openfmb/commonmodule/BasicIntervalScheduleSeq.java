...
/**
* A sequence of BasicIntervalSchedule instances.
*/
public final class BasicIntervalScheduleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BasicIntervalScheduleSeq() {
        super(BasicIntervalSchedule.class);
    }

    public BasicIntervalScheduleSeq (int initialMaximum) {
        super(BasicIntervalSchedule.class, initialMaximum);
    }

    public BasicIntervalScheduleSeq (Collection elements) {
        super(BasicIntervalSchedule.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, BasicIntervalSchedule.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' BasicIntervalSchedule objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(BasicIntervalSchedule.create());
