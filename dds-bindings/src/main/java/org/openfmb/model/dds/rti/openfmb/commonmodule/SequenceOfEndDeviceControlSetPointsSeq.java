...
/**
* A sequence of SequenceOfEndDeviceControlSetPoints instances.
*/
public final class SequenceOfEndDeviceControlSetPointsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SequenceOfEndDeviceControlSetPointsSeq() {
        super(SequenceOfEndDeviceControlSetPoints.class);
    }

    public SequenceOfEndDeviceControlSetPointsSeq (int initialMaximum) {
        super(SequenceOfEndDeviceControlSetPoints.class, initialMaximum);
    }

    public SequenceOfEndDeviceControlSetPointsSeq (Collection elements) {
        super(SequenceOfEndDeviceControlSetPoints.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SequenceOfEndDeviceControlSetPoints.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SequenceOfEndDeviceControlSetPoints objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SequenceOfEndDeviceControlSetPoints.create());
