...
/**
* A sequence of LoadForecastModule instances.
*/
public final class LoadForecastModuleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public LoadForecastModuleSeq() {
        super(LoadForecastModule.class);
    }

    public LoadForecastModuleSeq (int initialMaximum) {
        super(LoadForecastModule.class, initialMaximum);
    }

    public LoadForecastModuleSeq (Collection elements) {
        super(LoadForecastModule.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, LoadForecastModule.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' LoadForecastModule objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(LoadForecastModule.create());
