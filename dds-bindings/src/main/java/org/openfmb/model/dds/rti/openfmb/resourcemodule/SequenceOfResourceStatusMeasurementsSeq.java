...
/**
* A sequence of SequenceOfResourceStatusMeasurements instances.
*/
public final class SequenceOfResourceStatusMeasurementsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SequenceOfResourceStatusMeasurementsSeq() {
        super(SequenceOfResourceStatusMeasurements.class);
    }

    public SequenceOfResourceStatusMeasurementsSeq (int initialMaximum) {
        super(SequenceOfResourceStatusMeasurements.class, initialMaximum);
    }

    public SequenceOfResourceStatusMeasurementsSeq (Collection elements) {
        super(SequenceOfResourceStatusMeasurements.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SequenceOfResourceStatusMeasurements.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SequenceOfResourceStatusMeasurements objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SequenceOfResourceStatusMeasurements.create());
