...
/**
* A sequence of SequenceOfOptimizedMicroGridMarketMarketRuns instances.
*/
public final class SequenceOfOptimizedMicroGridMarketMarketRunsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SequenceOfOptimizedMicroGridMarketMarketRunsSeq() {
        super(SequenceOfOptimizedMicroGridMarketMarketRuns.class);
    }

    public SequenceOfOptimizedMicroGridMarketMarketRunsSeq (int initialMaximum) {
        super(SequenceOfOptimizedMicroGridMarketMarketRuns.class, initialMaximum);
    }

    public SequenceOfOptimizedMicroGridMarketMarketRunsSeq (Collection elements) {
        super(SequenceOfOptimizedMicroGridMarketMarketRuns.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SequenceOfOptimizedMicroGridMarketMarketRuns.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SequenceOfOptimizedMicroGridMarketMarketRuns objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SequenceOfOptimizedMicroGridMarketMarketRuns.create());
