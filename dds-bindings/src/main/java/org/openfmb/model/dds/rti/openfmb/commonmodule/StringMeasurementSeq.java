...
/**
* A sequence of StringMeasurement instances.
*/
public final class StringMeasurementSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public StringMeasurementSeq() {
        super(StringMeasurement.class);
    }

    public StringMeasurementSeq (int initialMaximum) {
        super(StringMeasurement.class, initialMaximum);
    }

    public StringMeasurementSeq (Collection elements) {
        super(StringMeasurement.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, StringMeasurement.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' StringMeasurement objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(StringMeasurement.create());
