...
/**
* A sequence of IrregularTimePoint instances.
*/
public final class IrregularTimePointSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public IrregularTimePointSeq() {
        super(IrregularTimePoint.class);
    }

    public IrregularTimePointSeq (int initialMaximum) {
        super(IrregularTimePoint.class, initialMaximum);
    }

    public IrregularTimePointSeq (Collection elements) {
        super(IrregularTimePoint.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, IrregularTimePoint.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' IrregularTimePoint objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(IrregularTimePoint.create());
