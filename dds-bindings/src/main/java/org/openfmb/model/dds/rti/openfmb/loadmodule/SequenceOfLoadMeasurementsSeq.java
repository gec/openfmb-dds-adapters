...
/**
* A sequence of SequenceOfLoadMeasurements instances.
*/
public final class SequenceOfLoadMeasurementsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SequenceOfLoadMeasurementsSeq() {
        super(SequenceOfLoadMeasurements.class);
    }

    public SequenceOfLoadMeasurementsSeq (int initialMaximum) {
        super(SequenceOfLoadMeasurements.class, initialMaximum);
    }

    public SequenceOfLoadMeasurementsSeq (Collection elements) {
        super(SequenceOfLoadMeasurements.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SequenceOfLoadMeasurements.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SequenceOfLoadMeasurements objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SequenceOfLoadMeasurements.create());
