...
/**
* A sequence of WindData instances.
*/
public final class WindDataSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public WindDataSeq() {
        super(WindData.class);
    }

    public WindDataSeq (int initialMaximum) {
        super(WindData.class, initialMaximum);
    }

    public WindDataSeq (Collection elements) {
        super(WindData.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, WindData.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' WindData objects (beyond the original size of this object)
