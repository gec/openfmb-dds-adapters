...
/**
* A sequence of LoadReadingModule instances.
*/
public final class LoadReadingModuleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public LoadReadingModuleSeq() {
        super(LoadReadingModule.class);
    }

    public LoadReadingModuleSeq (int initialMaximum) {
        super(LoadReadingModule.class, initialMaximum);
    }

    public LoadReadingModuleSeq (Collection elements) {
        super(LoadReadingModule.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, LoadReadingModule.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' LoadReadingModule objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(LoadReadingModule.create());
