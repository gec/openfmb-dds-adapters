...
/**
* A sequence of SequenceOfBatterySystemControlSetPoints instances.
*/
public final class SequenceOfBatterySystemControlSetPointsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SequenceOfBatterySystemControlSetPointsSeq() {
        super(SequenceOfBatterySystemControlSetPoints.class);
    }

    public SequenceOfBatterySystemControlSetPointsSeq (int initialMaximum) {
        super(SequenceOfBatterySystemControlSetPoints.class, initialMaximum);
    }

    public SequenceOfBatterySystemControlSetPointsSeq (Collection elements) {
        super(SequenceOfBatterySystemControlSetPoints.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SequenceOfBatterySystemControlSetPoints.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SequenceOfBatterySystemControlSetPoints objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SequenceOfBatterySystemControlSetPoints.create());
