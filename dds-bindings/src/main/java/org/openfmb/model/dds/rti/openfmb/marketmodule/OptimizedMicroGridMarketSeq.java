...
/**
* A sequence of OptimizedMicroGridMarket instances.
*/
public final class OptimizedMicroGridMarketSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public OptimizedMicroGridMarketSeq() {
        super(OptimizedMicroGridMarket.class);
    }

    public OptimizedMicroGridMarketSeq (int initialMaximum) {
        super(OptimizedMicroGridMarket.class, initialMaximum);
    }

    public OptimizedMicroGridMarketSeq (Collection elements) {
        super(OptimizedMicroGridMarket.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, OptimizedMicroGridMarket.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' OptimizedMicroGridMarket objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(OptimizedMicroGridMarket.create());
