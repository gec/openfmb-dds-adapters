...
/**
* A sequence of RecloserControlProfile instances.
*/
public final class RecloserControlProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RecloserControlProfileSeq() {
        super(RecloserControlProfile.class);
    }

    public RecloserControlProfileSeq (int initialMaximum) {
        super(RecloserControlProfile.class, initialMaximum);
    }

    public RecloserControlProfileSeq (Collection elements) {
        super(RecloserControlProfile.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RecloserControlProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RecloserControlProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RecloserControlProfile.create());
