...
/**
* A sequence of BatteryEventModule instances.
*/
public final class BatteryEventModuleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BatteryEventModuleSeq() {
        super(BatteryEventModule.class);
    }

    public BatteryEventModuleSeq (int initialMaximum) {
        super(BatteryEventModule.class, initialMaximum);
    }

    public BatteryEventModuleSeq (Collection elements) {
        super(BatteryEventModule.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, BatteryEventModule.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' BatteryEventModule objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(BatteryEventModule.create());
