...
/**
* A sequence of GenerationForecastModule instances.
*/
public final class GenerationForecastModuleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public GenerationForecastModuleSeq() {
        super(GenerationForecastModule.class);
    }

    public GenerationForecastModuleSeq (int initialMaximum) {
        super(GenerationForecastModule.class, initialMaximum);
    }

    public GenerationForecastModuleSeq (Collection elements) {
        super(GenerationForecastModule.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, GenerationForecastModule.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' GenerationForecastModule objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(GenerationForecastModule.create());
