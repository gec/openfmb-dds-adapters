...
/**
* A sequence of SecurityEventModule instances.
*/
public final class SecurityEventModuleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SecurityEventModuleSeq() {
        super(SecurityEventModule.class);
    }

    public SecurityEventModuleSeq (int initialMaximum) {
        super(SecurityEventModule.class, initialMaximum);
    }

    public SecurityEventModuleSeq (Collection elements) {
        super(SecurityEventModule.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SecurityEventModule.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SecurityEventModule objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SecurityEventModule.create());
