...
/**
* A sequence of ResourceReadingModule instances.
*/
public final class ResourceReadingModuleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ResourceReadingModuleSeq() {
        super(ResourceReadingModule.class);
    }

    public ResourceReadingModuleSeq (int initialMaximum) {
        super(ResourceReadingModule.class, initialMaximum);
    }

    public ResourceReadingModuleSeq (Collection elements) {
        super(ResourceReadingModule.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ResourceReadingModule.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ResourceReadingModule objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ResourceReadingModule.create());
