...
/**
* A sequence of WeatherDataProfile instances.
*/
public final class WeatherDataProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public WeatherDataProfileSeq() {
        super(WeatherDataProfile.class);
    }

    public WeatherDataProfileSeq (int initialMaximum) {
        super(WeatherDataProfile.class, initialMaximum);
    }

    public WeatherDataProfileSeq (Collection elements) {
        super(WeatherDataProfile.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, WeatherDataProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' WeatherDataProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(WeatherDataProfile.create());
