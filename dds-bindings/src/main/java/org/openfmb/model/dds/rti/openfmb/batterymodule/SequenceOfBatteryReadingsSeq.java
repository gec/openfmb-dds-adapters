...
/**
* A sequence of SequenceOfBatteryReadings instances.
*/
public final class SequenceOfBatteryReadingsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SequenceOfBatteryReadingsSeq() {
        super(SequenceOfBatteryReadings.class);
    }

    public SequenceOfBatteryReadingsSeq (int initialMaximum) {
        super(SequenceOfBatteryReadings.class, initialMaximum);
    }

    public SequenceOfBatteryReadingsSeq (Collection elements) {
        super(SequenceOfBatteryReadings.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SequenceOfBatteryReadings.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SequenceOfBatteryReadings objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SequenceOfBatteryReadings.create());
