...
/**
* A sequence of SetPoint instances.
*/
public final class SetPointSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SetPointSeq() {
        super(SetPoint.class);
    }

    public SetPointSeq (int initialMaximum) {
        super(SetPoint.class, initialMaximum);
    }

    public SetPointSeq (Collection elements) {
        super(SetPoint.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SetPoint.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SetPoint objects (beyond the original size of this object)
