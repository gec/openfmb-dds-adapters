...
/**
* A sequence of InterchangeScheduleModule instances.
*/
public final class InterchangeScheduleModuleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public InterchangeScheduleModuleSeq() {
        super(InterchangeScheduleModule.class);
    }

    public InterchangeScheduleModuleSeq (int initialMaximum) {
        super(InterchangeScheduleModule.class, initialMaximum);
    }

    public InterchangeScheduleModuleSeq (Collection elements) {
        super(InterchangeScheduleModule.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, InterchangeScheduleModule.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' InterchangeScheduleModule objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(InterchangeScheduleModule.create());
