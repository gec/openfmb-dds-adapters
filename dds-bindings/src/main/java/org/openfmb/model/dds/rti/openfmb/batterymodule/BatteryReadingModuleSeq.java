...
/**
* A sequence of BatteryReadingModule instances.
*/
public final class BatteryReadingModuleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BatteryReadingModuleSeq() {
        super(BatteryReadingModule.class);
    }

    public BatteryReadingModuleSeq (int initialMaximum) {
        super(BatteryReadingModule.class, initialMaximum);
    }

    public BatteryReadingModuleSeq (Collection elements) {
        super(BatteryReadingModule.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, BatteryReadingModule.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' BatteryReadingModule objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(BatteryReadingModule.create());
