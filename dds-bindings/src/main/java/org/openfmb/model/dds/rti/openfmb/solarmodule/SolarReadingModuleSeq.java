...
/**
* A sequence of SolarReadingModule instances.
*/
public final class SolarReadingModuleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SolarReadingModuleSeq() {
        super(SolarReadingModule.class);
    }

    public SolarReadingModuleSeq (int initialMaximum) {
        super(SolarReadingModule.class, initialMaximum);
    }

    public SolarReadingModuleSeq (Collection elements) {
        super(SolarReadingModule.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SolarReadingModule.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SolarReadingModule objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SolarReadingModule.create());
