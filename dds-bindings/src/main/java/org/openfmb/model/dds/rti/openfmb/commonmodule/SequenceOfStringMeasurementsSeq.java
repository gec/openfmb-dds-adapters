...
/**
* A sequence of SequenceOfStringMeasurements instances.
*/
public final class SequenceOfStringMeasurementsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SequenceOfStringMeasurementsSeq() {
        super(SequenceOfStringMeasurements.class);
    }

    public SequenceOfStringMeasurementsSeq (int initialMaximum) {
        super(SequenceOfStringMeasurements.class, initialMaximum);
    }

    public SequenceOfStringMeasurementsSeq (Collection elements) {
        super(SequenceOfStringMeasurements.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SequenceOfStringMeasurements.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SequenceOfStringMeasurements objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SequenceOfStringMeasurements.create());
