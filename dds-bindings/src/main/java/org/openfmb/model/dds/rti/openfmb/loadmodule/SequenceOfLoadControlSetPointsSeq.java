...
/**
* A sequence of SequenceOfLoadControlSetPoints instances.
*/
public final class SequenceOfLoadControlSetPointsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SequenceOfLoadControlSetPointsSeq() {
        super(SequenceOfLoadControlSetPoints.class);
    }

    public SequenceOfLoadControlSetPointsSeq (int initialMaximum) {
        super(SequenceOfLoadControlSetPoints.class, initialMaximum);
    }

    public SequenceOfLoadControlSetPointsSeq (Collection elements) {
        super(SequenceOfLoadControlSetPoints.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SequenceOfLoadControlSetPoints.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SequenceOfLoadControlSetPoints objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SequenceOfLoadControlSetPoints.create());
