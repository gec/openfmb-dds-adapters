...
/**
* A sequence of BatterySystemControl instances.
*/
public final class BatterySystemControlSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BatterySystemControlSeq() {
        super(BatterySystemControl.class);
    }

    public BatterySystemControlSeq (int initialMaximum) {
        super(BatterySystemControl.class, initialMaximum);
    }

    public BatterySystemControlSeq (Collection elements) {
        super(BatterySystemControl.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, BatterySystemControl.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' BatterySystemControl objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(BatterySystemControl.create());
