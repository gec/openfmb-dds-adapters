...
/**
* A sequence of ExecutionTypeKind instances.
*/
public final class ExecutionTypeKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ExecutionTypeKindSeq() {
        super(ExecutionTypeKind.class);
    }

    public ExecutionTypeKindSeq (int initialMaximum) {
        super(ExecutionTypeKind.class, initialMaximum);
    }

    public ExecutionTypeKindSeq (Collection elements) {
        super(ExecutionTypeKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ExecutionTypeKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ExecutionTypeKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ExecutionTypeKind.create());
