...
/**
* A sequence of SolarForecastProfile instances.
*/
public final class SolarForecastProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SolarForecastProfileSeq() {
        super(SolarForecastProfile.class);
    }

    public SolarForecastProfileSeq (int initialMaximum) {
        super(SolarForecastProfile.class, initialMaximum);
    }

    public SolarForecastProfileSeq (Collection elements) {
        super(SolarForecastProfile.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SolarForecastProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SolarForecastProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SolarForecastProfile.create());
