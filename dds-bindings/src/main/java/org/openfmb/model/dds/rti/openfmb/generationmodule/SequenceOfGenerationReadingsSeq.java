...
/**
* A sequence of SequenceOfGenerationReadings instances.
*/
public final class SequenceOfGenerationReadingsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SequenceOfGenerationReadingsSeq() {
        super(SequenceOfGenerationReadings.class);
    }

    public SequenceOfGenerationReadingsSeq (int initialMaximum) {
        super(SequenceOfGenerationReadings.class, initialMaximum);
    }

    public SequenceOfGenerationReadingsSeq (Collection elements) {
        super(SequenceOfGenerationReadings.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SequenceOfGenerationReadings.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SequenceOfGenerationReadings objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SequenceOfGenerationReadings.create());
