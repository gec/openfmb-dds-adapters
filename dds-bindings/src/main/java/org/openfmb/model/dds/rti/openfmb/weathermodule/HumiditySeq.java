...
/**
* A sequence of Humidity instances.
*/
public final class HumiditySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public HumiditySeq (int initialMaximum) {
        super(Humidity.class, initialMaximum);
    }

    public HumiditySeq (Collection elements) {
        super(Humidity.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, Humidity.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' Humidity objects (beyond the original size of this object)
