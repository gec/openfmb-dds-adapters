...
/**
* A sequence of SequenceOfMarketMarketFactors instances.
*/
public final class SequenceOfMarketMarketFactorsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SequenceOfMarketMarketFactorsSeq() {
        super(SequenceOfMarketMarketFactors.class);
    }

    public SequenceOfMarketMarketFactorsSeq (int initialMaximum) {
        super(SequenceOfMarketMarketFactors.class, initialMaximum);
    }

    public SequenceOfMarketMarketFactorsSeq (Collection elements) {
        super(SequenceOfMarketMarketFactors.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SequenceOfMarketMarketFactors.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SequenceOfMarketMarketFactors objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SequenceOfMarketMarketFactors.create());
