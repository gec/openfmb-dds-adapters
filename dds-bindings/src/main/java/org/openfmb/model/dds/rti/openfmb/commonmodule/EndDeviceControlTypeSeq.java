...
/**
* A sequence of EndDeviceControlType instances.
*/
public final class EndDeviceControlTypeSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public EndDeviceControlTypeSeq() {
        super(EndDeviceControlType.class);
    }

    public EndDeviceControlTypeSeq (int initialMaximum) {
        super(EndDeviceControlType.class, initialMaximum);
    }

    public EndDeviceControlTypeSeq (Collection elements) {
        super(EndDeviceControlType.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, EndDeviceControlType.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' EndDeviceControlType objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(EndDeviceControlType.create());
