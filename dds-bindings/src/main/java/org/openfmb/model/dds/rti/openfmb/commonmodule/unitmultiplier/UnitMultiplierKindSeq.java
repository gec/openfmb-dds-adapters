...
package org.openfmb.model.dds.rti.openfmb.commonmodule.unitmultiplier;
...
/**
* A sequence of UnitMultiplierKind instances.
*/
public final class UnitMultiplierKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public UnitMultiplierKindSeq() {
        super(UnitMultiplierKind.class);
    }

    public UnitMultiplierKindSeq (int initialMaximum) {
        super(UnitMultiplierKind.class, initialMaximum);
    }

    public UnitMultiplierKindSeq (Collection elements) {
        super(UnitMultiplierKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, UnitMultiplierKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' UnitMultiplierKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(UnitMultiplierKind.create());
