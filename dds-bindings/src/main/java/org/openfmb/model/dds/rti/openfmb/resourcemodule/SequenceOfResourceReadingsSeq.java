...
/**
* A sequence of SequenceOfResourceReadings instances.
*/
public final class SequenceOfResourceReadingsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SequenceOfResourceReadingsSeq() {
        super(SequenceOfResourceReadings.class);
    }

    public SequenceOfResourceReadingsSeq (int initialMaximum) {
        super(SequenceOfResourceReadings.class, initialMaximum);
    }

    public SequenceOfResourceReadingsSeq (Collection elements) {
        super(SequenceOfResourceReadings.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SequenceOfResourceReadings.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SequenceOfResourceReadings objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SequenceOfResourceReadings.create());
