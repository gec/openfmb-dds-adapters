...
/**
* A sequence of LoadReadingProfile instances.
*/
public final class LoadReadingProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public LoadReadingProfileSeq() {
        super(LoadReadingProfile.class);
    }

    public LoadReadingProfileSeq (int initialMaximum) {
        super(LoadReadingProfile.class, initialMaximum);
    }

    public LoadReadingProfileSeq (Collection elements) {
        super(LoadReadingProfile.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, LoadReadingProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' LoadReadingProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(LoadReadingProfile.create());
