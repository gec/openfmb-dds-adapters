...
/**
* A sequence of SolarCapabilityProfile instances.
*/
public final class SolarCapabilityProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SolarCapabilityProfileSeq() {
        super(SolarCapabilityProfile.class);
    }

    public SolarCapabilityProfileSeq (int initialMaximum) {
        super(SolarCapabilityProfile.class, initialMaximum);
    }

    public SolarCapabilityProfileSeq (Collection elements) {
        super(SolarCapabilityProfile.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SolarCapabilityProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SolarCapabilityProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SolarCapabilityProfile.create());
