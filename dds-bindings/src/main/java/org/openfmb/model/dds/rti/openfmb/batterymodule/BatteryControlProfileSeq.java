...
/**
* A sequence of BatteryControlProfile instances.
*/
public final class BatteryControlProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BatteryControlProfileSeq() {
        super(BatteryControlProfile.class);
    }

    public BatteryControlProfileSeq (int initialMaximum) {
        super(BatteryControlProfile.class, initialMaximum);
    }

    public BatteryControlProfileSeq (Collection elements) {
        super(BatteryControlProfile.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, BatteryControlProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' BatteryControlProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(BatteryControlProfile.create());
