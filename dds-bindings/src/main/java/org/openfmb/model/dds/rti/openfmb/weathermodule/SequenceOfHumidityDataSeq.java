...
/**
* A sequence of SequenceOfHumidityData instances.
*/
public final class SequenceOfHumidityDataSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SequenceOfHumidityDataSeq() {
        super(SequenceOfHumidityData.class);
    }

    public SequenceOfHumidityDataSeq (int initialMaximum) {
        super(SequenceOfHumidityData.class, initialMaximum);
    }

    public SequenceOfHumidityDataSeq (Collection elements) {
        super(SequenceOfHumidityData.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SequenceOfHumidityData.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SequenceOfHumidityData objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SequenceOfHumidityData.create());
