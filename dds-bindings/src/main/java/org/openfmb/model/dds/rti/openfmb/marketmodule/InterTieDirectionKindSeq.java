...
/**
* A sequence of InterTieDirectionKind instances.
*/
public final class InterTieDirectionKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public InterTieDirectionKindSeq() {
        super(InterTieDirectionKind.class);
    }

    public InterTieDirectionKindSeq (int initialMaximum) {
        super(InterTieDirectionKind.class, initialMaximum);
    }

    public InterTieDirectionKindSeq (Collection elements) {
        super(InterTieDirectionKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, InterTieDirectionKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' InterTieDirectionKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(InterTieDirectionKind.create());
