...
/**
* A sequence of RecloserControlModule instances.
*/
public final class RecloserControlModuleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RecloserControlModuleSeq() {
        super(RecloserControlModule.class);
    }

    public RecloserControlModuleSeq (int initialMaximum) {
        super(RecloserControlModule.class, initialMaximum);
    }

    public RecloserControlModuleSeq (Collection elements) {
        super(RecloserControlModule.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RecloserControlModule.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RecloserControlModule objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RecloserControlModule.create());
