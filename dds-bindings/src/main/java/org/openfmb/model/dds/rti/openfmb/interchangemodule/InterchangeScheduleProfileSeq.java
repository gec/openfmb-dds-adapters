...
/**
* A sequence of InterchangeScheduleProfile instances.
*/
public final class InterchangeScheduleProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public InterchangeScheduleProfileSeq() {
        super(InterchangeScheduleProfile.class);
    }

    public InterchangeScheduleProfileSeq (int initialMaximum) {
        super(InterchangeScheduleProfile.class, initialMaximum);
    }

    public InterchangeScheduleProfileSeq (Collection elements) {
        super(InterchangeScheduleProfile.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, InterchangeScheduleProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' InterchangeScheduleProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(InterchangeScheduleProfile.create());
