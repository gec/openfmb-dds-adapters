...
/**
* A sequence of GenerationReadingModule instances.
*/
public final class GenerationReadingModuleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public GenerationReadingModuleSeq() {
        super(GenerationReadingModule.class);
    }

    public GenerationReadingModuleSeq (int initialMaximum) {
        super(GenerationReadingModule.class, initialMaximum);
    }

    public GenerationReadingModuleSeq (Collection elements) {
        super(GenerationReadingModule.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, GenerationReadingModule.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' GenerationReadingModule objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(GenerationReadingModule.create());
