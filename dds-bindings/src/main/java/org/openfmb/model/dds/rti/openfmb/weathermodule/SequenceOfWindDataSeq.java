...
/**
* A sequence of SequenceOfWindData instances.
*/
public final class SequenceOfWindDataSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SequenceOfWindDataSeq() {
        super(SequenceOfWindData.class);
    }

    public SequenceOfWindDataSeq (int initialMaximum) {
        super(SequenceOfWindData.class, initialMaximum);
    }

    public SequenceOfWindDataSeq (Collection elements) {
        super(SequenceOfWindData.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SequenceOfWindData.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SequenceOfWindData objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SequenceOfWindData.create());
