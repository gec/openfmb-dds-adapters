...
/**
* A sequence of SolarControlProfile instances.
*/
public final class SolarControlProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SolarControlProfileSeq() {
        super(SolarControlProfile.class);
    }

    public SolarControlProfileSeq (int initialMaximum) {
        super(SolarControlProfile.class, initialMaximum);
    }

    public SolarControlProfileSeq (Collection elements) {
        super(SolarControlProfile.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SolarControlProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SolarControlProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SolarControlProfile.create());
