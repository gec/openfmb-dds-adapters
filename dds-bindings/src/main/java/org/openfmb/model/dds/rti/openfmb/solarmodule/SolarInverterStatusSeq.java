...
/**
* A sequence of SolarInverterStatus instances.
*/
public final class SolarInverterStatusSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SolarInverterStatusSeq() {
        super(SolarInverterStatus.class);
    }

    public SolarInverterStatusSeq (int initialMaximum) {
        super(SolarInverterStatus.class, initialMaximum);
    }

    public SolarInverterStatusSeq (Collection elements) {
        super(SolarInverterStatus.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SolarInverterStatus.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SolarInverterStatus objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SolarInverterStatus.create());
