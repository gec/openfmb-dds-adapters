...
package org.openfmb.model.dds.rti.openfmb.commonmodule.flowdirection;
...
/**
* A sequence of FlowDirectionKind instances.
*/
public final class FlowDirectionKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public FlowDirectionKindSeq() {
        super(FlowDirectionKind.class);
    }

    public FlowDirectionKindSeq (int initialMaximum) {
        super(FlowDirectionKind.class, initialMaximum);
    }

    public FlowDirectionKindSeq (Collection elements) {
        super(FlowDirectionKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, FlowDirectionKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' FlowDirectionKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(FlowDirectionKind.create());
