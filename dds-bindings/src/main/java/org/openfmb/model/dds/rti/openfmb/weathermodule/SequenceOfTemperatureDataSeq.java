...
/**
* A sequence of SequenceOfTemperatureData instances.
*/
public final class SequenceOfTemperatureDataSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SequenceOfTemperatureDataSeq() {
        super(SequenceOfTemperatureData.class);
    }

    public SequenceOfTemperatureDataSeq (int initialMaximum) {
        super(SequenceOfTemperatureData.class, initialMaximum);
    }

    public SequenceOfTemperatureDataSeq (Collection elements) {
        super(SequenceOfTemperatureData.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SequenceOfTemperatureData.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SequenceOfTemperatureData objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SequenceOfTemperatureData.create());
