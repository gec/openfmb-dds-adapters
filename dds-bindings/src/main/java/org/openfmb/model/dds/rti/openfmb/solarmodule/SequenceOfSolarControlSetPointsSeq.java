...
/**
* A sequence of SequenceOfSolarControlSetPoints instances.
*/
public final class SequenceOfSolarControlSetPointsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SequenceOfSolarControlSetPointsSeq() {
        super(SequenceOfSolarControlSetPoints.class);
    }

    public SequenceOfSolarControlSetPointsSeq (int initialMaximum) {
        super(SequenceOfSolarControlSetPoints.class, initialMaximum);
    }

    public SequenceOfSolarControlSetPointsSeq (Collection elements) {
        super(SequenceOfSolarControlSetPoints.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SequenceOfSolarControlSetPoints.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SequenceOfSolarControlSetPoints objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SequenceOfSolarControlSetPoints.create());
