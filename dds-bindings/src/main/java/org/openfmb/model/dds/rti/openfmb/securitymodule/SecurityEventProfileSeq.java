...
/**
* A sequence of SecurityEventProfile instances.
*/
public final class SecurityEventProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SecurityEventProfileSeq() {
        super(SecurityEventProfile.class);
    }

    public SecurityEventProfileSeq (int initialMaximum) {
        super(SecurityEventProfile.class, initialMaximum);
    }

    public SecurityEventProfileSeq (Collection elements) {
        super(SecurityEventProfile.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SecurityEventProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SecurityEventProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SecurityEventProfile.create());
