...
/**
* A sequence of SolarControlModule instances.
*/
public final class SolarControlModuleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SolarControlModuleSeq() {
        super(SolarControlModule.class);
    }

    public SolarControlModuleSeq (int initialMaximum) {
        super(SolarControlModule.class, initialMaximum);
    }

    public SolarControlModuleSeq (Collection elements) {
        super(SolarControlModule.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SolarControlModule.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SolarControlModule objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SolarControlModule.create());
