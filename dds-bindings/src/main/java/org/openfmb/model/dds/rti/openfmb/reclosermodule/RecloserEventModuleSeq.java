...
/**
* A sequence of RecloserEventModule instances.
*/
public final class RecloserEventModuleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RecloserEventModuleSeq() {
        super(RecloserEventModule.class);
    }

    public RecloserEventModuleSeq (int initialMaximum) {
        super(RecloserEventModule.class, initialMaximum);
    }

    public RecloserEventModuleSeq (Collection elements) {
        super(RecloserEventModule.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RecloserEventModule.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RecloserEventModule objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RecloserEventModule.create());
