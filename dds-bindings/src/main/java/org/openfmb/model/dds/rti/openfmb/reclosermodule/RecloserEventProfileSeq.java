...
/**
* A sequence of RecloserEventProfile instances.
*/
public final class RecloserEventProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RecloserEventProfileSeq() {
        super(RecloserEventProfile.class);
    }

    public RecloserEventProfileSeq (int initialMaximum) {
        super(RecloserEventProfile.class, initialMaximum);
    }

    public RecloserEventProfileSeq (Collection elements) {
        super(RecloserEventProfile.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RecloserEventProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RecloserEventProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RecloserEventProfile.create());
