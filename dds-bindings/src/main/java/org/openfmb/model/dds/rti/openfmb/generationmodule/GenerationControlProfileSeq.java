...
/**
* A sequence of GenerationControlProfile instances.
*/
public final class GenerationControlProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public GenerationControlProfileSeq() {
        super(GenerationControlProfile.class);
    }

    public GenerationControlProfileSeq (int initialMaximum) {
        super(GenerationControlProfile.class, initialMaximum);
    }

    public GenerationControlProfileSeq (Collection elements) {
        super(GenerationControlProfile.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, GenerationControlProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' GenerationControlProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(GenerationControlProfile.create());
