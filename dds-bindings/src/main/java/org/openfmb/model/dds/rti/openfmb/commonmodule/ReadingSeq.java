...
/**
* A sequence of Reading instances.
*/
public final class ReadingSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ReadingSeq (int initialMaximum) {
        super(Reading.class, initialMaximum);
    }

    public ReadingSeq (Collection elements) {
        super(Reading.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, Reading.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' Reading objects (beyond the original size of this object)
