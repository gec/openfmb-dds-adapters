...
/**
* A sequence of SequenceOfLoadControlReadings instances.
*/
public final class SequenceOfLoadControlReadingsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SequenceOfLoadControlReadingsSeq() {
        super(SequenceOfLoadControlReadings.class);
    }

    public SequenceOfLoadControlReadingsSeq (int initialMaximum) {
        super(SequenceOfLoadControlReadings.class, initialMaximum);
    }

    public SequenceOfLoadControlReadingsSeq (Collection elements) {
        super(SequenceOfLoadControlReadings.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SequenceOfLoadControlReadings.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SequenceOfLoadControlReadings objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SequenceOfLoadControlReadings.create());
