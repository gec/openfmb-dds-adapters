...
/**
* A sequence of Container instances.
*/
public final class ContainerSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ContainerSeq (int initialMaximum) {
        super(Container.class, initialMaximum);
    }

    public ContainerSeq (Collection elements) {
        super(Container.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, Container.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' Container objects (beyond the original size of this object)
