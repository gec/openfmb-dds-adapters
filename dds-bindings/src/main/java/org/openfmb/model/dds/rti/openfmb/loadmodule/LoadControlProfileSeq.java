...
/**
* A sequence of LoadControlProfile instances.
*/
public final class LoadControlProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public LoadControlProfileSeq() {
        super(LoadControlProfile.class);
    }

    public LoadControlProfileSeq (int initialMaximum) {
        super(LoadControlProfile.class, initialMaximum);
    }

    public LoadControlProfileSeq (Collection elements) {
        super(LoadControlProfile.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, LoadControlProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' LoadControlProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(LoadControlProfile.create());
