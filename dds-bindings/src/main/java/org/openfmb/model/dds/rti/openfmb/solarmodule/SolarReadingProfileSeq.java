...
/**
* A sequence of SolarReadingProfile instances.
*/
public final class SolarReadingProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SolarReadingProfileSeq() {
        super(SolarReadingProfile.class);
    }

    public SolarReadingProfileSeq (int initialMaximum) {
        super(SolarReadingProfile.class, initialMaximum);
    }

    public SolarReadingProfileSeq (Collection elements) {
        super(SolarReadingProfile.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SolarReadingProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SolarReadingProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SolarReadingProfile.create());
