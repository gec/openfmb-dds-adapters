...
/**
* A sequence of InterchangeSchedule instances.
*/
public final class InterchangeScheduleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public InterchangeScheduleSeq() {
        super(InterchangeSchedule.class);
    }

    public InterchangeScheduleSeq (int initialMaximum) {
        super(InterchangeSchedule.class, initialMaximum);
    }

    public InterchangeScheduleSeq (Collection elements) {
        super(InterchangeSchedule.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, InterchangeSchedule.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' InterchangeSchedule objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(InterchangeSchedule.create());
