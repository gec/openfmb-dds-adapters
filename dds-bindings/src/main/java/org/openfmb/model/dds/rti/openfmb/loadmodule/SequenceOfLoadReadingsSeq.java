...
/**
* A sequence of SequenceOfLoadReadings instances.
*/
public final class SequenceOfLoadReadingsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SequenceOfLoadReadingsSeq() {
        super(SequenceOfLoadReadings.class);
    }

    public SequenceOfLoadReadingsSeq (int initialMaximum) {
        super(SequenceOfLoadReadings.class, initialMaximum);
    }

    public SequenceOfLoadReadingsSeq (Collection elements) {
        super(SequenceOfLoadReadings.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SequenceOfLoadReadings.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SequenceOfLoadReadings objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SequenceOfLoadReadings.create());
