...
/**
* A sequence of SequenceOfMarketFactors instances.
*/
public final class SequenceOfMarketFactorsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SequenceOfMarketFactorsSeq() {
        super(SequenceOfMarketFactors.class);
    }

    public SequenceOfMarketFactorsSeq (int initialMaximum) {
        super(SequenceOfMarketFactors.class, initialMaximum);
    }

    public SequenceOfMarketFactorsSeq (Collection elements) {
        super(SequenceOfMarketFactors.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SequenceOfMarketFactors.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SequenceOfMarketFactors objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SequenceOfMarketFactors.create());
