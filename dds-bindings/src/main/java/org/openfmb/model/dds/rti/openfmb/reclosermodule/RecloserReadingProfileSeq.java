...
/**
* A sequence of RecloserReadingProfile instances.
*/
public final class RecloserReadingProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RecloserReadingProfileSeq() {
        super(RecloserReadingProfile.class);
    }

    public RecloserReadingProfileSeq (int initialMaximum) {
        super(RecloserReadingProfile.class, initialMaximum);
    }

    public RecloserReadingProfileSeq (Collection elements) {
        super(RecloserReadingProfile.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RecloserReadingProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RecloserReadingProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RecloserReadingProfile.create());
