...
/**
* A sequence of ResourceStatusProfile instances.
*/
public final class ResourceStatusProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ResourceStatusProfileSeq() {
        super(ResourceStatusProfile.class);
    }

    public ResourceStatusProfileSeq (int initialMaximum) {
        super(ResourceStatusProfile.class, initialMaximum);
    }

    public ResourceStatusProfileSeq (Collection elements) {
        super(ResourceStatusProfile.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ResourceStatusProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ResourceStatusProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ResourceStatusProfile.create());
