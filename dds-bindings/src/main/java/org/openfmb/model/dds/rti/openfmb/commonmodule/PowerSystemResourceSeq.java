...
/**
* A sequence of PowerSystemResource instances.
*/
public final class PowerSystemResourceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public PowerSystemResourceSeq() {
        super(PowerSystemResource.class);
    }

    public PowerSystemResourceSeq (int initialMaximum) {
        super(PowerSystemResource.class, initialMaximum);
    }

    public PowerSystemResourceSeq (Collection elements) {
        super(PowerSystemResource.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, PowerSystemResource.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' PowerSystemResource objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(PowerSystemResource.create());
