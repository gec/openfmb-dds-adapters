...
/**
* A sequence of SequenceOfMarketMarketRuns instances.
*/
public final class SequenceOfMarketMarketRunsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SequenceOfMarketMarketRunsSeq() {
        super(SequenceOfMarketMarketRuns.class);
    }

    public SequenceOfMarketMarketRunsSeq (int initialMaximum) {
        super(SequenceOfMarketMarketRuns.class, initialMaximum);
    }

    public SequenceOfMarketMarketRunsSeq (Collection elements) {
        super(SequenceOfMarketMarketRuns.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SequenceOfMarketMarketRuns.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SequenceOfMarketMarketRuns objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SequenceOfMarketMarketRuns.create());
