...
/**
* A sequence of GenerationControlModule instances.
*/
public final class GenerationControlModuleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public GenerationControlModuleSeq() {
        super(GenerationControlModule.class);
    }

    public GenerationControlModuleSeq (int initialMaximum) {
        super(GenerationControlModule.class, initialMaximum);
    }

    public GenerationControlModuleSeq (Collection elements) {
        super(GenerationControlModule.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, GenerationControlModule.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' GenerationControlModule objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(GenerationControlModule.create());
