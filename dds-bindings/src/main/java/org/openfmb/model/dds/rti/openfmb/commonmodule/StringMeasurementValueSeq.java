...
/**
* A sequence of StringMeasurementValue instances.
*/
public final class StringMeasurementValueSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public StringMeasurementValueSeq() {
        super(StringMeasurementValue.class);
    }

    public StringMeasurementValueSeq (int initialMaximum) {
        super(StringMeasurementValue.class, initialMaximum);
    }

    public StringMeasurementValueSeq (Collection elements) {
        super(StringMeasurementValue.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, StringMeasurementValue.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' StringMeasurementValue objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(StringMeasurementValue.create());
