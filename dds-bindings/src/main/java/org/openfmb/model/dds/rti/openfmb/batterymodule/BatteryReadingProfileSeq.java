...
/**
* A sequence of BatteryReadingProfile instances.
*/
public final class BatteryReadingProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BatteryReadingProfileSeq() {
        super(BatteryReadingProfile.class);
    }

    public BatteryReadingProfileSeq (int initialMaximum) {
        super(BatteryReadingProfile.class, initialMaximum);
    }

    public BatteryReadingProfileSeq (Collection elements) {
        super(BatteryReadingProfile.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, BatteryReadingProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' BatteryReadingProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(BatteryReadingProfile.create());
