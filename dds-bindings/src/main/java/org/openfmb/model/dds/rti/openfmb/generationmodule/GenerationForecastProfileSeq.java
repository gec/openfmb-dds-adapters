...
/**
* A sequence of GenerationForecastProfile instances.
*/
public final class GenerationForecastProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public GenerationForecastProfileSeq() {
        super(GenerationForecastProfile.class);
    }

    public GenerationForecastProfileSeq (int initialMaximum) {
        super(GenerationForecastProfile.class, initialMaximum);
    }

    public GenerationForecastProfileSeq (Collection elements) {
        super(GenerationForecastProfile.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, GenerationForecastProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' GenerationForecastProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(GenerationForecastProfile.create());
